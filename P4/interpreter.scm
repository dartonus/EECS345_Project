...
;debug feature: write 'var state;' in test code text file to break and return current state
...
; recursively search variable/method for object->parents
...
; helper function that checks whether a name is bound to a function/object
(define isfunc
  (lambda (name s)
...
;updating variables for function's scope/object's scope
;can't setbox properly, have to do it manually?
...
;declare a variable in current layer specifically 
...
;name tracking version of m_value
...
(define perform_with_scope
  ;perform version for func and class involves non-simple states
...
; helper method determining the actual object/class to be used in a dot call
(define getobject
...
; creates function's enviroments with params passed in
...
; helper detects if name is in current layer of state
(define inscope?
...
; stored in the same package format as function
...
; tentative classes parsed after toplevel runthrough:
...
;Classes get parsed and stored in a similar box to functions in the base layer
...
;executing the main function in the top layer 
(define m_run_main
  (lambda (state)
    (call/cc (lambda (return)
      (perform_func
...
;obtaining the super class name of given object instance name
(define getsuper
...
;Creating the package that a object name would be bound to
...
;Return the object's state with its varibles etc.
...
; (dot a x)
; also handles format such as (dot (new A) x)
  ...
  (lambda (line state instance)
    ...
    (if (list? (operand1 line))
        (cond
          ...
          (else (error 'error "shouldn't reach here"))
        )
