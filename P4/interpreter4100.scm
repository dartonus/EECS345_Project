...
(define m_manual_state
  (lambda (name exp s funcname instance)
        (if (eq? (lookup name s) "undefined")
          (error 'error "Use before declaration")
          (if (layered s)
            (if (isfunc exp s)
              (begin (set-box! (lookupbox name s) (lookup exp s)) s)
              (begin (set-box! (lookupbox name s) (m_value_scope exp (car s) funcname instance)) s))
            (begin (set-box! (lookupbox name s) (m_value_scope exp s funcname instance)) s)
          )
        )
      )
  )
  ...
  (lambda (expression s)
    ;(if (layered s) (m_value expression (car s))
      (cond
        ...
        ((eq? 'funcall  (operator expression)) (m_call_func 'main expression s 'null))
...
(define set_formal_params (lambda (params formals state funcscope funcname instance)
    (cond
      ...
      ((or (null? (cdr params)) (null? (cdr formals))) 
        (cons (m_manual_state
          (car formals)
          (if (isfunc (car params) state)
            (car params)
            (m_value_scope (car params) state funcname instance))
          funcscope
          funcname
          instance
          ) state))
