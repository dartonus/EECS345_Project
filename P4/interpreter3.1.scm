...
(define lookupforfunc
    (lambda (name s funcname)
      (if (not (layered s))
        ...
        (if (inscope? funcname s)
          (lookup name (car s))
          (lookupforfunc name (cdr s) funcname)
          )
  ...
  (lambda (expression s)
    ;(if (layered s) (m_value expression (car s))
      (cond
        ...
        ((eq? '= (operator expression)) (if (eq? "undefined" operand2) (error 'error "undefined") (m_value (m_value (operand2 expression) s) (m_state_funcscope expression s))))
...
(define perform_with_scope
  ...
  (lambda (line state break continue throw return)
      (cond
        ...
        ((eq? (operator line) '=) (m_state_funcscope line state))
        ;return needs revamp (immediate break)
        ((or (eq? (operator line) 'return) (eq? (operator line) 'throw)) 
                            (cond
                                    ((eq? (cadr line) 'state) (return state))
...
(define func_blockhandler
  (lambda (line s break continue throw return)
            (cond 
              ((empty? (cdr line)) (perform_with_scope (car line)  s (lambda (v) (break (cdr v))) (lambda (v) (continue (cdr v))) (lambda (v) (throw (cdr v))) return)) 
              (else 
                (func_blockhandler (cdr line) (perform_with_scope (car line)  s (lambda (v) (break (cdr v))) (lambda (v) (continue (cdr v))) (lambda (v) (throw (cdr v))) return) break continue throw return)))
